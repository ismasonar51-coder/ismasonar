...
function saveSession(code, playerId, isHost){
  try{
    localStorage.setItem(SESSION_KEY, JSON.stringify({ roomCode: code, playerId, isHost, savedAt: Date.now() }));
...
async function tryRestoreSession(){
  ...
  if(!session.savedAt || Date.now() - session.savedAt > THREE_HOURS){
    clearSession();
    return false;
  }
  ...
  try{
    ...
    if(room.status === "lobby"){
      renderSongChips(session.isHost ? "host" : "player");
...
// ---------- Connexion Google (identité host + accès playlists YouTube) ----------
async function handleGoogleSignIn(){
  ...
  try{
    ...
    console.error(err);
    notify("Erreur de connexion Google, réessaie.");
  }
}
...
function updateAuthUI(){
  ...
  const tabPlaylistsBtn = document.getElementById("tab-btn-playlists");
  const playlistsSigninPrompt = document.getElementById("playlists-signin-prompt");

  if(currentUser){
    signedOutEl.classList.add("hidden");
    signedInEl.classList.remove("hidden");
    document.getElementById("google-account-name").textContent = currentUser.displayName;
    document.getElementById("google-account-photo").src = currentUser.photoURL || "";
  ...
  if(tabPlaylistsBtn) tabPlaylistsBtn.classList.toggle("hidden", !youtubeAccessToken);
  if(playlistsSigninPrompt) playlistsSigninPrompt.classList.toggle("hidden", !!youtubeAccessToken);
}

// Note : Firebase garde la connexion (identité) après un rafraîchissement,
// mais le jeton d'accès YouTube lui, ne survit pas — il faut se reconnecter
// pour retrouver l'accès aux playlists après un rechargement de page.
onAuthStateChanged(auth, user => {
  currentUser = user;
  updateAuthUI();
});

// ---------- Favoris locaux (fonctionnent sans compte) ----------
const FAVORITES_KEY = "ismasonar-favorites";
...
function isFavorite(videoId){
  return getFavorites().some(f => f.youtubeId === videoId);
}

function toggleFavorite(song){
  let favs = getFavorites();
  if(favs.some(f => f.youtubeId === song.youtubeId)){
    favs = favs.filter(f => f.youtubeId !== song.youtubeId);
  } else {
    favs.unshift(song);
  }
  try{ localStorage.setItem(FAVORITES_KEY, JSON.stringify(favs)); } catch(err){ /* ignore */ }
}
...
function renderFavoritesList(){
  ...
  if(favs.length === 0){
    el.innerHTML = `<li class="song-result-status">Aucun favori pour l'instant. Ajoute-en avec l'étoile ☆ sur un résultat.</li>`;
    return;
  }
  el.innerHTML = "";
  favs.forEach(f => el.appendChild(buildResultRow(f.youtubeId, f.title, "", f.thumbnail || "")));
}

// ---------- Ligne de résultat réutilisable (recherche / playlists / favoris) ----------
function buildResultRow(videoId, title, subtitle, thumbnail){
  const li = document.createElement("li");
  li.className = "song-result-big";
  const fav = isFavorite(videoId);
  li.innerHTML = `
    <img src="${thumbnail}" alt="">
    <span><strong>${escapeHtml(title)}</strong><small>${escapeHtml(subtitle)}</small></span>
    <button type="button" class="song-result-star ${fav ? "active" : ""}">${fav ? "★" : "☆"}</button>
  `;
  li.addEventListener("click", () => addToSearchSlot(videoId, title, thumbnail));
  const starBtn = li.querySelector(".song-result-star");
  starBtn.addEventListener("click", e => {
    e.stopPropagation();
    toggleFavorite({ youtubeId: videoId, title, thumbnail });
    const nowFav = isFavorite(videoId);
...
// ---------- Playlists YouTube de l'utilisateur connecté ----------
async function fetchMyPlaylists(){
  const listEl = document.getElementById("my-playlists-list");
  listEl.innerHTML = `<li class="song-result-status">Chargement de tes playlists...</li>`;
  ...
  try{
    ...
    if(data.error){
      listEl.innerHTML = `<li class="song-result-status">Erreur d'accès, reconnecte-toi et réessaie.</li>`;
      return;
    }
    if(!data.items || data.items.length === 0){
      listEl.innerHTML = `<li class="song-result-status">Aucune playlist trouvée sur ton compte.</li>`;
      return;
    }
    ...
    data.items.forEach(pl => {
      const li = document.createElement("li");
      li.className = "song-result-big";
      const thumb = pl.snippet.thumbnails.medium || pl.snippet.thumbnails.default;
      li.innerHTML = `
        <img src="${thumb ? thumb.url : ""}" alt="">
        <span><strong>${escapeHtml(pl.snippet.title)}</strong><small>${pl.contentDetails.itemCount} musiques</small></span>
      `;
    ...
    console.error(err);
    listEl.innerHTML = `<li class="song-result-status">Erreur de chargement, réessaie.</li>`;
  }
}

async function openPlaylist(playlistId){
  document.getElementById("playlists-list-view").classList.add("hidden");
  document.getElementById("playlist-items-view").classList.remove("hidden");
  const itemsEl = document.getElementById("playlist-items-list");
  itemsEl.innerHTML = `<li class="song-result-status">Chargement...</li>`;

  try{
    const res = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${playlistId}&maxResults=25`, {
      headers: { Authorization: `Bearer ${youtubeAccessToken}` }
    });
    const data = await res.json();

    if(!data.items || data.items.length === 0){
      itemsEl.innerHTML = `<li class="song-result-status">Playlist vide.</li>`;
      return;
    }
    ...
    data.items.forEach(item => {
      ...
      const thumbnail = thumbs ? (thumbs.medium || thumbs.default).url : "";
      itemsEl.appendChild(buildResultRow(videoId, title, item.snippet.videoOwnerChannelTitle || "", thumbnail));
    });
  } catch(err){
    console.error(err);
    itemsEl.innerHTML = `<li class="song-result-status">Erreur de chargement.</li>`;
  }
}
window.openPlaylist = openPlaylist;

function backToPlaylistsList(){
  document.getElementById("playlist-items-view").classList.add("hidden");
  document.getElementById("playlists-list-view").classList.remove("hidden");
}
window.backToPlaylistsList = backToPlaylistsList;

// ---------- Recherche de musiques (page dédiée) ----------
...
let songSearchRole = null;
let songSearchSlots = [null, null];

function renderSongChips(role){
  const el = document.getElementById(`song-chips-${role}`);
  if(!el) return;
  const [s1, s2] = pendingSongs[role];
  el.innerHTML = [s1, s2].map((s, i) => s
    ? `<div class="song-chip">🎵 ${escapeHtml(s.title)}</div>`
    : `<div class="song-chip empty">Musique ${i + 1} non choisie</div>`
  ).join("");
}

function openSongSearch(role){
  songSearchRole = role;
  songSearchSlots = [pendingSongs[role][0], pendingSongs[role][1]];
  document.getElementById("page-search-query").value = "";
  document.getElementById("page-search-results").innerHTML = "";
  renderSearchSlots();
  switchSearchTab("search");
  document.getElementById("tab-btn-playlists").classList.toggle("hidden", !youtubeAccessToken);
  showView("view-song-search");
}
window.openSongSearch = openSongSearch;

function closeSongSearch(){
  showView(songSearchRole === "host" ? "view-host-lobby" : "view-player-lobby");
}
window.closeSongSearch = closeSongSearch;

function renderSearchSlots(){
  const el = document.getElementById("search-selection-slots");
  el.innerHTML = [0, 1].map(i => {
    const song = songSearchSlots[i];
    if(song){
      return `
        <div class="search-slot filled">
          <button type="button" class="slot-remove" onclick="removeSearchSlot(${i})" title="Retirer">✕</button>
          <img src="${song.thumbnail || ""}" alt="">
          <span class="slot-title">${escapeHtml(song.title)}</span>
        </div>
      `;
    }
    return `<div class="search-slot">Musique ${i + 1}<br>Vide</div>`;
  }).join("");

  document.getElementById("confirm-search-btn").classList.toggle("hidden", !(songSearchSlots[0] && songSearchSlots[1]));
}

function removeSearchSlot(i){
  songSearchSlots[i] = null;
  renderSearchSlots();
}
window.removeSearchSlot = removeSearchSlot;

async function runPageSearch(){
  const query = document.getElementById("page-search-query").value.trim();
  ...
  try{
    // videoCategoryId=10 = catégorie "Musique" sur YouTube : filtre déjà pas mal
    // de vlogs/gaming/etc., mais ne garantit pas à 100% "officiel" vs cover/fan-made.
    const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoCategoryId=10&videoEmbeddable=true&maxResults=10&safeSearch=moderate&q=${encodeURIComponent(query)}&key=${YOUTUBE_API_KEY}`;
    ...
    data.items.forEach(item => {
      ...
      const thumbs = item.snippet.thumbnails;
      const thumbnail = (thumbs.medium || thumbs.default).url;
      resultsEl.appendChild(buildResultRow(videoId, title, channelTitle, thumbnail));
    });
  } catch(err){
    console.error(err);
...
window.runPageSearch = runPageSearch;

function addToSearchSlot(videoId, title, thumbnail){
  if(songSearchSlots.some(s => s && s.youtubeId === videoId)){
    notify("Tu as déjà choisi cette musique.");
    return;
  }
  const emptyIndex = songSearchSlots.findIndex(s => !s);
  if(emptyIndex === -1){
    notify("Tu as déjà tes 2 musiques. Retire-en une (✕) pour la remplacer.");
    return;
  }
  songSearchSlots[emptyIndex] = { youtubeId: videoId, title, thumbnail };
  renderSearchSlots();
}

async function confirmSongSearch(){
  if(!songSearchSlots[0] || !songSearchSlots[1]) return;
  pendingSongs[songSearchRole][0] = { youtubeId: songSearchSlots[0].youtubeId, title: songSearchSlots[0].title };
  pendingSongs[songSearchRole][1] = { youtubeId: songSearchSlots[1].youtubeId, title: songSearchSlots[1].title };
  renderSongChips(songSearchRole);
  await submitSongs(songSearchRole);
  showView(songSearchRole === "host" ? "view-host-lobby" : "view-player-lobby");
}
window.confirmSongSearch = confirmSongSearch;
...
function enterSongPhase(room, isHost, sameRoom){
  ...
  if(sameRoom && !isHost){
    ...
    startCountdown(room.phaseStartedAt, "round-countdown", onTimerEnd, false);
  } else {
    hostOnlyCaption.classList.add("hidden");
    playerCaption.classList.remove("hidden");

    const onStarted = () => {
      startCountdown(Date.now(), "round-countdown", onTimerEnd, true);
    };

    if(audioUnlocked){
      document.getElementById("round-countdown").textContent = "…";
      playRoundSong(round.youtubeId, onStarted);
    } else {
...
function enterDebatePhase(room, isHost, sameRoom){
  ...
  // On précharge déjà la musique de la manche suivante pendant qu'on débat
  if(isHost || !sameRoom){
    preloadNextSong(room);
  }
}
...
function computeRanksWithTies(players){
...
function enterFinalView(){
  ...
  const eligible = currentPlayersCache.filter(p => p.isPlaying !== false);
  const ranked = computeRanksWithTies(eligible);

  document.getElementById("final-ranking").innerHTML = ranked.map(p => `
    <li class="final-row">
      <span class="final-medal">${medalForRank(p.rank)}</span>
  ...
  const badges = [];
  const maxCorrect = Math.max(0, ...eligible.map(p => p.correctGuesses || 0));
  if(maxCorrect > 0){
    const names = eligible.filter(p => (p.correctGuesses || 0) === maxCorrect).map(p => escapeHtml(p.name));
    badges.push(`🕵️ Meilleur limier : ${names.join(", ")}`);
  }
  const maxStealth = Math.max(0, ...eligible.map(p => p.stealthPoints || 0));
  if(maxStealth > 0){
    const names = eligible.filter(p => (p.stealthPoints || 0) === maxStealth).map(p => escapeHtml(p.name));
    badges.push(`🎭 Roi du camouflage : ${names.join(", ")}`);
  }
...
async function playRoundSong(videoId, onStarted){
  await ytApiReadyPromise;

  const attachStateListener = (player) => {
    const listener = e => {
  ...
  if(ytPlayer){
    ...
    attachStateListener(ytPlayer);
    if(preloadedVideoId === videoId){
      ytPlayer.playVideo(); // déjà préchargée, démarrage quasi instantané
    } else {
    ...
    ytPlayer = new YT.Player("yt-player-container", {
      ...
      events: {
        ...
        onStateChange: e => {
          if(e.data === YT.PlayerState.PLAYING && onStarted) onStarted();
        }
      }
    });
  }
...
// ---------- Préchargement de la musique suivante (pendant le débat) ----------
async function preloadNextSong(room){
  const nextRound = room.roundOrder[room.currentRoundIndex + 1];
  if(!nextRound || preloadedVideoId === nextRound.youtubeId) return;
  await ytApiReadyPromise;

  if(ytPlayer){
    ytPlayer.mute();
    ytPlayer.cueVideoById(nextRound.youtubeId);
  } else {
    ytPlayer = new YT.Player("yt-player-container", {
      height: "1",
      width: "1",
      videoId: nextRound.youtubeId,
      playerVars: { controls: 0, disablekb: 1, modestbranding: 1, rel: 0 },
      events: { onReady: e => e.target.mute() }
    });
  }
  preloadedVideoId = nextRound.youtubeId;
}
