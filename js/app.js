...
const WARMUP_VIDEO_ID = "jNQXAC9IVRw";
let spectatorRoomCache = null;
let spectatorPlayersCache = []; // vidéo courte et fiable, sert juste à "débloquer" le son
...
async function handleCreateRoom(event){
  ...
  try{
    ...
    updateSpectatorButtonVisibility();
...
async function searchYouTube(role, slot){
  ...
  try{
    ...
    const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoCategoryId=10&videoEmbeddable=true&maxResults=6&safeSearch=moderate&q=${encodeURIComponent(query)}&key=${YOUTUBE_API_KEY}`;
    ...
    data.items.forEach(item => {
      ...
      li.innerHTML = `
        ...
        <span>
          ...
          <small>${escapeHtml(channelTitle)}</small>
        </span>
      `;
...
// ---------- Fenêtre spectateur (lecture seule, pour projection) ----------
function buildSpectatorUrl(code){
  return `${window.location.origin}${window.location.pathname}?room=${code}&role=spectator`;
}

function openSpectatorWindow(){
  window.open(buildSpectatorUrl(currentRoomCode), "_blank");
}
window.openSpectatorWindow = openSpectatorWindow;

function updateSpectatorButtonVisibility(){
  const btn = document.getElementById("open-spectator-btn");
  if(!btn) return;
  btn.classList.toggle("hidden", !(sameRoomValue && hostPlays));
}

function checkSpectatorLink(){
  const params = new URLSearchParams(window.location.search);
  if(params.get("role") !== "spectator") return false;
  const code = params.get("room");
  if(!code) return false;
  enterSpectatorMode(code);
  return true;
}

function enterSpectatorMode(code){
  document.getElementById("spectator-room-code").textContent = code;
  showView("view-spectator");

  onSnapshot(collection(db, "rooms", code, "players"), snapshot => {
    spectatorPlayersCache = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
    renderSpectatorContent();
  });

  onSnapshot(doc(db, "rooms", code), snap => {
    spectatorRoomCache = snap.data();
    renderSpectatorContent();
  });
}

function renderSpectatorContent(){
  const el = document.getElementById("spectator-content");
  const room = spectatorRoomCache;
  const players = spectatorPlayersCache;
  if(!room){ el.innerHTML = `<p class="spectator-sub">Chargement...</p>`; return; }

  if(room.status === "lobby"){
    const ready = players.filter(p => p.songsSubmitted).length;
    el.innerHTML = `
      <p class="spectator-big">En attente du lancement...</p>
      <p class="spectator-sub">${players.length} joueur${players.length > 1 ? "s" : ""} connecté${players.length > 1 ? "s" : ""} · ${ready} prêt${ready > 1 ? "s" : ""}</p>
    `;
  } else if(room.status === "playing"){
    const total = room.roundOrder.length;
    const current = room.currentRoundIndex + 1;

    if(room.roundPhase === "song"){
      el.innerHTML = `<p class="spectator-big">🎵 Manche ${current} / ${total}</p><p class="spectator-sub">Écoutez bien...</p>`;
    } else if(room.roundPhase === "debate"){
      el.innerHTML = `<p class="spectator-big">💬 Débat — Manche ${current} / ${total}</p><p class="spectator-sub">Qui a choisi cette musique ?</p>`;
    } else if(room.roundPhase === "voting"){
      const votedCount = Object.keys(room.votes || {}).length;
      const totalActive = room.activePlayerIds.length;
      el.innerHTML = `<p class="spectator-big">🗳️ Vote en cours</p><p class="spectator-sub">${votedCount} / ${totalActive} ont voté</p>`;
    } else if(room.roundPhase === "results"){
      const result = room.lastResult;
      if(!result) return;
      const owner = players.find(p => p.id === result.ownerId);
      const rows = players
        .filter(p => room.activePlayerIds.includes(p.id))
        .sort((a, b) => (b.score || 0) - (a.score || 0))
        .map(p => `<li class="spectator-row"><span>${escapeHtml(p.name)}</span><span>${p.score || 0} pts</span></li>`)
        .join("");
      el.innerHTML = `
        <p class="spectator-big">🎵 « ${escapeHtml(result.songTitle)} »</p>
        <p class="spectator-sub">choisie par <strong>${owner ? escapeHtml(owner.name) : "?"}</strong> (+${result.ownerPoints} pts, ${result.correctCount}/${result.guesserCount} l'ont trouvé)</p>
        <ul class="spectator-scores">${rows}</ul>
      `;
    }
  } else if(room.status === "finished"){
    const eligible = players.filter(p => p.isPlaying !== false);
    const ranked = computeRanksWithTies(eligible);
    const rows = ranked.map(p =>
      `<li class="spectator-row"><span>${medalForRank(p.rank)} ${escapeHtml(p.name)}</span><span>${p.score || 0} pts</span></li>`
    ).join("");
    el.innerHTML = `<p class="spectator-big">🏁 Partie terminée !</p><ul class="spectator-scores">${rows}</ul>`;
  }
}
